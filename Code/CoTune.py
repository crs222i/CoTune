...
global_max_stagnation = int(sys.argv[2])
path = os.getenv("RESULT_PATH", "results/")
type = "best_solution"
...
class Proposition:
    ...
    def evaluate_proposition(self, population: List[Solution], left, right):
        ...
        # 计算对数密度
        log_d = kde.score_samples(x_plot)  # log p(x)
        p = np.exp(log_d)  # p(x)

        # 4) 在 [left,right] 上截取
        dx = x_plot[1, 0] - x_plot[0, 0]
        mask = (x_plot[:, 0] >= left) & (x_plot[:, 0] <= right)
        p_int = p[mask]

        # 5) 归一化密度，使 ∫_left^right p_norm(x)dx = 1
        Z = np.sum(p_int) * dx
        p_norm = p_int / (Z + 1e-16)

        # 6) 计算归一化后的微分熵：H = -∫ p_norm log p_norm dx
        entropy = -np.sum(p_norm * np.log(p_norm + 1e-16)) * dx

        # 7) 存到 fitness 并返回
        self.fitness = float(entropy)
...
class GeneticAlgorithm():
    ...
    def handle_stagnation_case(self, data, _):
        ...
        with open(path + f"fragments_output{_}.txt", "a") as file:
    ...
    def iteration2(self, _):
        ...
        while not self.termination_criterion.is_met and budget < max_budget and live > 0:
            ...
            if stagnation_count > max_stagnation:
                ...
                self.handle_stagnation_case(self.population_2, _)
                self.population_2 = self.evaluate_solution2(self.population_2)
                trigger = 2
                stagnation_count = 0  # 重置停滞计数器
...
if __name__ == "__main__":
    _ = int(sys.argv[1]) if len(sys.argv) > 1 else -1
    problem = PropositionSingleObjProblem(os.getenv("CSV", "none"))
    data = os.getenv("DATA", "[]")
